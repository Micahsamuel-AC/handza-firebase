interface LogoProps { size?: number; variant?: "full"|"icon"|"wordmark"; theme?: "light"|"dark"; className?: string; }
export default function HANDZALogo({ size=40, variant="full", theme="light", className="" }: LogoProps) {
  ...
  if (variant==="wordmark") return <span className={`font-heading font-bold ${className}`} style={{color:textColor,fontSize:size*0.5}}>HANDZA</span>;
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <Icon/>
      <div className="flex flex-col">
        <span className="font-heading font-bold leading-none" style={{color:textColor,fontSize:size*0.45}}>HANDZA</span>
        {size>=40 && <span className="font-body leading-none mt-0.5" style={{color:theme==="dark"?"rgba(255,255,255,0.5)":"#888780",fontSize:size*0.18}}>Connecting the right hands</span>}
      </div>
    </div>
  );
}
